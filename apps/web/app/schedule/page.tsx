...
interface Appt { id: string; startAt: string; status: string; type: string; service?: { name: string } | null; patient: { user: { firstName: string; lastName: string; phone: string } } }
...
export default function SchedulePage() {
  ...
  const loadAgenda = useCallback(async () => {
    ...
    setAppts(await api.get<Appt[]>(`/me/agenda?from=${from}&to=${to}`));
  }, [agendaDate]);
  ...
  return (
    <AppShell>
      <h1 className="mb-6 text-2xl font-bold">Mi agenda</h1>
